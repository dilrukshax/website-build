import React from 'react';

// Header
import HeaderV1 from './components/header/v1';
import HeaderV2 from './components/header/v2';
import HeaderV3 from './components/header/v3';
import HeaderV4 from './components/header/v4';
import HeaderV5 from './components/header/v5';
import HeaderV6 from './components/header/v6';
import HeaderV7 from './components/header/v7';
import HeaderV8 from './components/header/v8';
import HeaderV9 from './components/header/v9';
import HeaderV10 from './components/header/v10';
import HeaderV11 from './components/header/v11';
import HeaderV12 from './components/header/v12';

// Hero
import HeroV1 from './components/hero/v1';
import HeroV2 from './components/hero/v2';
import HeroV3 from './components/hero/v3';
import HeroV4 from './components/hero/v4';
import HeroV5 from './components/hero/v5';
import HeroV6 from './components/hero/v6';
import HeroV7 from './components/hero/v7';
import HeroV8 from './components/hero/v8';
import HeroV9 from './components/hero/v9';
import HeroV10 from './components/hero/v10';
import HeroV11 from './components/hero/v11';
import HeroV12 from './components/hero/v12';

// About
import AboutV1 from './components/about/v1';
import AboutV2 from './components/about/v2';
import AboutV3 from './components/about/v3';
import AboutV4 from './components/about/v4';
import AboutV5 from './components/about/v5';
import AboutV6 from './components/about/v6';
import AboutV7 from './components/about/v7';
import AboutV8 from './components/about/v8';
import AboutV9 from './components/about/v9';
import AboutV10 from './components/about/v10';
import AboutV11 from './components/about/v11';
import AboutV12 from './components/about/v12';

// Services
import ServicesV1 from './components/services/v1';
import ServicesV2 from './components/services/v2';
import ServicesV3 from './components/services/v3';
import ServicesV4 from './components/services/v4';
import ServicesV5 from './components/services/v5';
import ServicesV6 from './components/services/v6';
import ServicesV7 from './components/services/v7';
import ServicesV8 from './components/services/v8';
import ServicesV9 from './components/services/v9';
import ServicesV10 from './components/services/v10';
import ServicesV11 from './components/services/v11';
import ServicesV12 from './components/services/v12';

// Products
import ProductV1 from './components/product/v1';
import ProductV2 from './components/product/v2';
import ProductV3 from './components/product/v3';
import ProductV4 from './components/product/v4';
import ProductV5 from './components/product/v5';
import ProductV6 from './components/product/v6';
import ProductV7 from './components/product/v7';
import ProductV8 from './components/product/v8';
import ProductV9 from './components/product/v9';
import ProductV10 from './components/product/v10';
import ProductV11 from './components/product/v11';
import ProductV12 from './components/product/v12';

// Logos
import LogosV5 from './components/logos/v5';
import LogosV6 from './components/logos/v6';
import LogosV7 from './components/logos/v7';
import LogosV8 from './components/logos/v8';
import LogosV9 from './components/logos/v9';
import LogosV10 from './components/logos/v10';
import LogosV11 from './components/logos/v11';
import LogosV12 from './components/logos/v12';

// Gallery
import GalleryV1 from './components/gallery/v1';
import GalleryV2 from './components/gallery/v2';
import GalleryV3 from './components/gallery/v3';
import GalleryV4 from './components/gallery/v4';
import GalleryV5 from './components/gallery/v5';
import GalleryV6 from './components/gallery/v6';
import GalleryV7 from './components/gallery/v7';
import GalleryV8 from './components/gallery/v8';
import GalleryV9 from './components/gallery/v9';
import GalleryV10 from './components/gallery/v10';
import GalleryV11 from './components/gallery/v11';
import GalleryV12 from './components/gallery/v12';

// Testimonials
import TestimonialsV1 from './components/testimonials/v1';
import TestimonialsV2 from './components/testimonials/v2';
import TestimonialsV3 from './components/testimonials/v3';
import TestimonialsV4 from './components/testimonials/v4';
import TestimonialsV5 from './components/testimonials/v5';
import TestimonialsV6 from './components/testimonials/v6';
import TestimonialsV7 from './components/testimonials/v7';
import TestimonialsV8 from './components/testimonials/v8';
import TestimonialsV9 from './components/testimonials/v9';
import TestimonialsV10 from './components/testimonials/v10';
import TestimonialsV11 from './components/testimonials/v11';
import TestimonialsV12 from './components/testimonials/v12';

// Contact
import ContactV1 from './components/contact/v1';
import ContactV2 from './components/contact/v2';
import ContactV3 from './components/contact/v3';
import ContactV4 from './components/contact/v4';
import ContactV5 from './components/contact/v5';
import ContactV6 from './components/contact/v6';
import ContactV7 from './components/contact/v7';
import ContactV8 from './components/contact/v8';
import ContactV9 from './components/contact/v9';
import ContactV10 from './components/contact/v10';
import ContactV11 from './components/contact/v11';
import ContactV12 from './components/contact/v12';

// Footer
import FooterV1 from './components/footer/v1';
import FooterV2 from './components/footer/v2';
import FooterV3 from './components/footer/v3';
import FooterV4 from './components/footer/v4';
import FooterV5 from './components/footer/v5';
import FooterV6 from './components/footer/v6';
import FooterV7 from './components/footer/v7';
import FooterV8 from './components/footer/v8';
import FooterV9 from './components/footer/v9';
import FooterV10 from './components/footer/v10';
import FooterV11 from './components/footer/v11';
import FooterV12 from './components/footer/v12';

// Booking Widget
import BookingWidgetV1 from './components/booking-widget/v1';
import BookingWidgetV2 from './components/booking-widget/v2';
import BookingWidgetV3 from './components/booking-widget/v3';
import BookingWidgetV4 from './components/booking-widget/v4';
import BookingWidgetV5 from './components/booking-widget/v5';
import BookingWidgetV6 from './components/booking-widget/v6';
import BookingWidgetV7 from './components/booking-widget/v7';
import BookingWidgetV8 from './components/booking-widget/v8';
import BookingWidgetV9 from './components/booking-widget/v9';
import BookingWidgetV10 from './components/booking-widget/v10';
import BookingWidgetV11 from './components/booking-widget/v11';
import BookingWidgetV12 from './components/booking-widget/v12';

// Pricing
import PricingV1 from './components/pricing/v1';
import PricingV2 from './components/pricing/v2';
import PricingV3 from './components/pricing/v3';
import PricingV4 from './components/pricing/v4';
import PricingV5 from './components/pricing/v5';
import PricingV6 from './components/pricing/v6';
import PricingV7 from './components/pricing/v7';
import PricingV8 from './components/pricing/v8';
import PricingV9 from './components/pricing/v9';
import PricingV10 from './components/pricing/v10';
import PricingV11 from './components/pricing/v11';
import PricingV12 from './components/pricing/v12';

// FAQ
import FAQV1 from './components/faq/v1';
import FAQV2 from './components/faq/v2';
import FAQV3 from './components/faq/v3';
import FAQV4 from './components/faq/v4';
import FAQV5 from './components/faq/v5';
import FAQV6 from './components/faq/v6';
import FAQV7 from './components/faq/v7';
import FAQV8 from './components/faq/v8';
import FAQV9 from './components/faq/v9';
import FAQV10 from './components/faq/v10';
import FAQV11 from './components/faq/v11';
import FAQV12 from './components/faq/v12';

// Team
import TeamV1 from './components/team/v1';
import TeamV2 from './components/team/v2';
import TeamV3 from './components/team/v3';
import TeamV4 from './components/team/v4';
import TeamV5 from './components/team/v5';
import TeamV6 from './components/team/v6';
import TeamV7 from './components/team/v7';
import TeamV8 from './components/team/v8';
import TeamV9 from './components/team/v9';
import TeamV10 from './components/team/v10';
import TeamV11 from './components/team/v11';
import TeamV12 from './components/team/v12';

import type { ThemeComponentProps } from './types';

type ThemeComponent = React.ComponentType<ThemeComponentProps>;

const VERSIONED_KEY_PATTERN = /^([a-z-]+)\/v(\d+)$/;
const FEATURE_SLUG_ALIASES: Record<string, string> = {
    products: 'product',
    service: 'services',
    testimonial: 'testimonials',
    logo: 'logos',
};

const MAX_REGISTERED_VERSION_BY_FEATURE: Record<string, number> = {
    about: 12,
    'booking-widget': 12,
    contact: 12,
    faq: 12,
    footer: 12,
    gallery: 12,
    header: 12,
    hero: 12,
    pricing: 12,
    product: 12,
    services: 12,
    team: 12,
    testimonials: 12,
};

const TARGET_THEME_VERSION = 12;

export const THEME_REGISTRY: Record<string, ThemeComponent> = {
    // Header
    'header/v1': HeaderV1,
    'header/v2': HeaderV2,
    'header/v3': HeaderV3,
    'header/v4': HeaderV4,
    'header/v5': HeaderV5,
    'header/v6': HeaderV6,
    'header/v7': HeaderV7,
    'header/v8': HeaderV8,
    'header/v9': HeaderV9,
    'header/v10': HeaderV10,
    'header/v11': HeaderV11,
    'header/v12': HeaderV12,

    // Hero
    'hero/v1': HeroV1,
    'hero/v2': HeroV2,
    'hero/v3': HeroV3,
    'hero/v4': HeroV4,
    'hero/v5': HeroV5,
    'hero/v6': HeroV6,
    'hero/v7': HeroV7,
    'hero/v8': HeroV8,
    'hero/v9': HeroV9,
    'hero/v10': HeroV10,
    'hero/v11': HeroV11,
    'hero/v12': HeroV12,

    // About
    'about/v1': AboutV1,
    'about/v2': AboutV2,
    'about/v3': AboutV3,
    'about/v4': AboutV4,
    'about/v5': AboutV5,
    'about/v6': AboutV6,
    'about/v7': AboutV7,
    'about/v8': AboutV8,
    'about/v9': AboutV9,
    'about/v10': AboutV10,
    'about/v11': AboutV11,
    'about/v12': AboutV12,

    // Services
    'services/v1': ServicesV1,
    'services/v2': ServicesV2,
    'services/v3': ServicesV3,
    'services/v4': ServicesV4,
    'services/v5': ServicesV5,
    'services/v6': ServicesV6,
    'services/v7': ServicesV7,
    'services/v8': ServicesV8,
    'services/v9': ServicesV9,
    'services/v10': ServicesV10,
    'services/v11': ServicesV11,
    'services/v12': ServicesV12,

    // Products
    'product/v1': ProductV1,
    'product/v2': ProductV2,
    'product/v3': ProductV3,
    'product/v4': ProductV4,
    'product/v5': ProductV5,
    'product/v6': ProductV6,
    'product/v7': ProductV7,
    'product/v8': ProductV8,
    'product/v9': ProductV9,
    'product/v10': ProductV10,
    'product/v11': ProductV11,
    'product/v12': ProductV12,

    // Logos
    'logos/v5': LogosV5,
    'logos/v6': LogosV6,
    'logos/v7': LogosV7,
    'logos/v8': LogosV8,
    'logos/v9': LogosV9,
    'logos/v10': LogosV10,
    'logos/v11': LogosV11,
    'logos/v12': LogosV12,

    // Gallery
    'gallery/v1': GalleryV1,
    'gallery/v2': GalleryV2,
    'gallery/v3': GalleryV3,
    'gallery/v4': GalleryV4,
    'gallery/v5': GalleryV5,
    'gallery/v6': GalleryV6,
    'gallery/v7': GalleryV7,
    'gallery/v8': GalleryV8,
    'gallery/v9': GalleryV9,
    'gallery/v10': GalleryV10,
    'gallery/v11': GalleryV11,
    'gallery/v12': GalleryV12,

    // Testimonials
    'testimonials/v1': TestimonialsV1,
    'testimonials/v2': TestimonialsV2,
    'testimonials/v3': TestimonialsV3,
    'testimonials/v4': TestimonialsV4,
    'testimonials/v5': TestimonialsV5,
    'testimonials/v6': TestimonialsV6,
    'testimonials/v7': TestimonialsV7,
    'testimonials/v8': TestimonialsV8,
    'testimonials/v9': TestimonialsV9,
    'testimonials/v10': TestimonialsV10,
    'testimonials/v11': TestimonialsV11,
    'testimonials/v12': TestimonialsV12,

    // Contact
    'contact/v1': ContactV1,
    'contact/v2': ContactV2,
    'contact/v3': ContactV3,
    'contact/v4': ContactV4,
    'contact/v5': ContactV5,
    'contact/v6': ContactV6,
    'contact/v7': ContactV7,
    'contact/v8': ContactV8,
    'contact/v9': ContactV9,
    'contact/v10': ContactV10,
    'contact/v11': ContactV11,
    'contact/v12': ContactV12,

    // Footer
    'footer/v1': FooterV1,
    'footer/v2': FooterV2,
    'footer/v3': FooterV3,
    'footer/v4': FooterV4,
    'footer/v5': FooterV5,
    'footer/v6': FooterV6,
    'footer/v7': FooterV7,
    'footer/v8': FooterV8,
    'footer/v9': FooterV9,
    'footer/v10': FooterV10,
    'footer/v11': FooterV11,
    'footer/v12': FooterV12,

    // Booking Widget
    'booking-widget/v1': BookingWidgetV1,
    'booking-widget/v2': BookingWidgetV2,
    'booking-widget/v3': BookingWidgetV3,
    'booking-widget/v4': BookingWidgetV4,
    'booking-widget/v5': BookingWidgetV5,
    'booking-widget/v6': BookingWidgetV6,
    'booking-widget/v7': BookingWidgetV7,
    'booking-widget/v8': BookingWidgetV8,
    'booking-widget/v9': BookingWidgetV9,
    'booking-widget/v10': BookingWidgetV10,
    'booking-widget/v11': BookingWidgetV11,
    'booking-widget/v12': BookingWidgetV12,

    // Pricing
    'pricing/v1': PricingV1,
    'pricing/v2': PricingV2,
    'pricing/v3': PricingV3,
    'pricing/v4': PricingV4,
    'pricing/v5': PricingV5,
    'pricing/v6': PricingV6,
    'pricing/v7': PricingV7,
    'pricing/v8': PricingV8,
    'pricing/v9': PricingV9,
    'pricing/v10': PricingV10,
    'pricing/v11': PricingV11,
    'pricing/v12': PricingV12,

    // FAQ
    'faq/v1': FAQV1,
    'faq/v2': FAQV2,
    'faq/v3': FAQV3,
    'faq/v4': FAQV4,
    'faq/v5': FAQV5,
    'faq/v6': FAQV6,
    'faq/v7': FAQV7,
    'faq/v8': FAQV8,
    'faq/v9': FAQV9,
    'faq/v10': FAQV10,
    'faq/v11': FAQV11,
    'faq/v12': FAQV12,

    // Team
    'team/v1': TeamV1,
    'team/v2': TeamV2,
    'team/v3': TeamV3,
    'team/v4': TeamV4,
    'team/v5': TeamV5,
    'team/v6': TeamV6,
    'team/v7': TeamV7,
    'team/v8': TeamV8,
    'team/v9': TeamV9,
    'team/v10': TeamV10,
    'team/v11': TeamV11,
    'team/v12': TeamV12,
};

for (const [featureSlug, maxVersion] of Object.entries(MAX_REGISTERED_VERSION_BY_FEATURE)) {
    for (let version = maxVersion + 1; version <= TARGET_THEME_VERSION; version += 1) {
        const sourceVersion = ((version - 1) % maxVersion) + 1;
        const sourceKey = `${featureSlug}/v${sourceVersion}`;
        const targetKey = `${featureSlug}/v${version}`;
        const sourceComponent = THEME_REGISTRY[sourceKey];
        if (sourceComponent) {
            THEME_REGISTRY[targetKey] = sourceComponent;
        }
    }
}

const LATEST_REGISTERED_KEY_BY_FEATURE: Record<string, string> = Object.keys(THEME_REGISTRY).reduce(
    (acc, key) => {
        const match = key.match(VERSIONED_KEY_PATTERN);
        if (!match) {
            return acc;
        }

        const featureSlug = match[1];
        const versionToken = match[2];
        if (!featureSlug || !versionToken) {
            return acc;
        }

        const version = Number(versionToken);
        const currentKey = acc[featureSlug];

        if (!currentKey) {
            acc[featureSlug] = key;
            return acc;
        }

        const currentVersionMatch = currentKey.match(VERSIONED_KEY_PATTERN);
        const currentVersion = currentVersionMatch ? Number(currentVersionMatch[2]) : -1;
        if (version > currentVersion) {
            acc[featureSlug] = key;
        }

        return acc;
    },
    {} as Record<string, string>,
);

function normalizeFeatureSlug(raw: string): string {
    const normalized = raw.trim().toLowerCase().replace(/\s+/g, '-');
    return FEATURE_SLUG_ALIASES[normalized] || normalized;
}

function normalizeComponentKey(raw: string): string {
    if (typeof raw !== 'string') {
        return '';
    }

    const normalized = raw
        .normalize('NFKC')
        .replace(/[\u200B-\u200D\u2060\uFEFF]/g, '')
        .replace(/[\\／⁄∕]/g, '/')
        .replace(/[\x00-\x1F\x7F]/g, '')
        .trim()
        .toLowerCase()
        .replace(/\s*\/\s*/g, '/')
        .replace(/\s+/g, '-');

    const match = normalized.match(VERSIONED_KEY_PATTERN);
    if (!match) {
        return normalized;
    }

    const featureSlugToken = match[1];
    const versionToken = match[2];
    if (!featureSlugToken || !versionToken) {
        return normalized;
    }

    const featureSlug = normalizeFeatureSlug(featureSlugToken);
    return `${featureSlug}/v${versionToken}`;
}

export function getThemeComponent(componentKey: string): ThemeComponent | null {
    const normalizedKey = normalizeComponentKey(componentKey);
    if (!normalizedKey) {
        return null;
    }

    const direct = THEME_REGISTRY[normalizedKey];
    if (direct) {
        return direct;
    }

    const match = normalizedKey.match(VERSIONED_KEY_PATTERN);
    if (!match) {
        return null;
    }

    const featureSlugToken = match[1];
    if (!featureSlugToken) {
        return null;
    }

    const featureSlug = normalizeFeatureSlug(featureSlugToken);
    const fallbackKey = LATEST_REGISTERED_KEY_BY_FEATURE[featureSlug];
    if (!fallbackKey) {
        return null;
    }

    return THEME_REGISTRY[fallbackKey] || null;
}

export function getRegisteredKeys(): string[] {
    return Object.keys(THEME_REGISTRY);
}
