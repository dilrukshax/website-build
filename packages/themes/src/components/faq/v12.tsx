...
export default function FAQV12({ content, tokens }: ThemeComponentProps) {
    ...
    <div style={{ maxWidth: '940px', margin: '0 auto', border, background: card, borderRadius: '16px', padding: '14px' }}>
        <h2 style={{ margin: 0, color: titleColor }}>{title}</h2>
        <p style={{ margin: '8px 0 0 0', color: muted }}>{subtitle}</p>
        <div style={{ marginTop: '10px', display: 'grid', gap: '8px' }}>
            {faqs.map((item, index) => (
                <article key={index} style={{ border, borderRadius: '10px', padding: '10px' }}>
