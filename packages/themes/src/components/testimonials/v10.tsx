...
export default function TestimonialsV10({ content, tokens }: ThemeComponentProps) {
    ...
    return (
        <section id="reviews" style={{ background: sectionBg, padding: 'clamp(72px, 10vw, 120px) 16px', fontFamily: tokens.font }}>
            <div style={{ maxWidth: '1080px', margin: '0 auto', border, borderRadius: '16px', padding: '12px' }}><h2 style={{ margin: 0, color: titleColor }}>{title}</h2><div style={{ marginTop: '10px', display: 'grid', gap: '10px', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))' }}>{items.map((item, i) => <article key={i} style={{ border, borderRadius: '10px', padding: '10px' }}><p style={{ margin: 0, color: titleColor }}>{item.quote}</p><p style={{ margin: '6px 0 0 0', color: tokens.primary }}>{item.author}</p></article>)}</div></div>
        </section>
    );
}
