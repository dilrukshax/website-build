...
export default function TestimonialsV11({ content, tokens }: ThemeComponentProps) {
    ...
    return (
        <section id="reviews" style={{ background: sectionBg, padding: 'clamp(72px, 10vw, 120px) 16px', fontFamily: tokens.font }}>
            <div style={{ maxWidth: '1120px', margin: '0 auto', display: 'grid', gap: '10px', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))' }}>{items.map((item, i) => <article key={i} style={{ border, background: card, borderRadius: '12px', padding: '10px' }}><p style={{ margin: 0, color: tokens.primary, fontSize: '12px', fontWeight: 800 }}>Review {i + 1}</p><p style={{ margin: '8px 0 0 0', color: titleColor }}>&ldquo;{item.quote}&rdquo;</p><p style={{ margin: '8px 0 0 0', color: muted }}>{item.author}</p></article>)}</div>
        </section>
    );
}
