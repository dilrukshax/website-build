...
export default function ServicesV8({ content, tokens, context }: ThemeComponentProps) {
    ...
    return (
        <section id="services" style={{ background: sectionBg, padding: 'clamp(72px, 10vw, 120px) 16px', fontFamily: tokens.font }}>
            <div style={{ maxWidth: '1040px', margin: '0 auto' }}><h2 style={{ margin: 0, color: titleColor, textAlign: 'center' }}>{title}</h2><div style={{ marginTop: '10px', display: 'flex', gap: '10px', overflowX: 'auto' }}>{visible.map((item, i) => <article key={item.id || i} style={{ flex: '0 0 280px', border, background: card, borderRadius: '14px', padding: '12px' }}><h3 style={{ margin: 0, color: titleColor }}>{item.title}</h3><p style={{ margin: '8px 0 0 0', color: muted }}>{item.description}</p></article>)}</div></div>
        </section>
    );
}
