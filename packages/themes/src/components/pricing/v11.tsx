...
export default function PricingV11({ content, tokens }: ThemeComponentProps) {
    ...
    return (
        <section id="pricing" style={{ background: sectionBg, padding: 'clamp(72px, 10vw, 120px) 16px', fontFamily: tokens.font }}>
            <div style={{ maxWidth: '1100px', margin: '0 auto' }}><h2 style={{ margin: 0, color: titleColor }}>{title}</h2><div style={{ marginTop: '10px', display: 'grid', gap: '10px', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))' }}>{plans.map((plan, i) => <article key={i} style={{ border, background: card, borderRadius: '12px', padding: '10px' }}><p style={{ margin: 0, color: tokens.primary, fontSize: '12px', fontWeight: 800 }}>Plan {i + 1}</p><h3 style={{ margin: '6px 0 0 0', color: titleColor }}>{plan.name}</h3><p style={{ margin: '6px 0 0 0', color: tokens.primary }}>{plan.price}{plan.period}</p></article>)}</div></div>
        </section>
    );
}
