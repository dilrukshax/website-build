...
export default function LogosV10({ content, tokens }: ThemeComponentProps) {
    ...
    return (
        <section style={{ background: sectionBg, padding: '52px 16px', fontFamily: tokens.font }}>
            ...
            <div style={{ maxWidth: '1120px', margin: '0 auto', display: 'flex', flexWrap: 'wrap', gap: '8px', justifyContent: 'space-between' }}>{logos.map((logo, i) => <span key={i} style={{ borderBottom: `2px solid ${tokens.primary}`, color: titleColor, minHeight: '30px', display: 'inline-flex', alignItems: 'center' }}>{logo.name}</span>)}</div>
        </section>
    );
}
