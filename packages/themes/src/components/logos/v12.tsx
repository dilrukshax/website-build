...
export default function LogosV12({ content, tokens }: ThemeComponentProps) {
    ...
    return (
        <section style={{ background: sectionBg, padding: '52px 16px', fontFamily: tokens.font }}>
            ...
            <div style={{ maxWidth: '980px', margin: '0 auto', border, borderRadius: '14px', padding: '12px' }}><div style={{ display: 'grid', gap: '8px', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))' }}>{logos.slice(0, 6).map((logo, i) => <div key={i} style={{ border, borderRadius: '10px', minHeight: '48px', display: 'grid', placeItems: 'center', color: titleColor }}>{logo.name}</div>)}</div></div>
        </section>
    );
}
