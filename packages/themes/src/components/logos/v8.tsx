...
export default function LogosV8({ content, tokens }: ThemeComponentProps) {
    ...
    return (
        <section style={{ background: sectionBg, padding: '52px 16px', fontFamily: tokens.font }}>
            ...
            <div style={{ maxWidth: '1120px', margin: '0 auto', display: 'grid', gap: '10px', gridTemplateColumns: 'repeat(6, minmax(0, 1fr))' }}>{logos.map((logo, i) => <article key={i} style={{ gridColumn: 'span 2', border, borderRadius: '12px', minHeight: '58px', display: 'grid', placeItems: 'center', color: titleColor }}>{logo.name}</article>)}</div>
        </section>
    );
}
