...
export default function HeaderV11({ content, tokens }: ThemeComponentProps) {
    const projectName = (content.projectName as string) || (content.businessName as string) || 'Fusion Growth';
    ...
    <header style={{ position: 'sticky', top: 0, zIndex: 1000, background: sectionBg, borderBottom: border, fontFamily: tokens.font }}>
        <div style={{ maxWidth: '1120px', margin: '0 auto', padding: '10px 16px', display: 'grid', gap: '8px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: '8px', alignItems: 'center' }}>
                ...
                <a href={ctaLink} style={{ background: tokens.primary, color: '#fff', borderRadius: '999px', minHeight: '36px', display: 'inline-flex', alignItems: 'center', padding: '0 12px', textDecoration: 'none' }}>{ctaText}</a>
            </div>
            <nav style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '8px' }}>{navItems.map((item) => <a key={item.label + item.href} href={item.href} style={{ color: muted, textDecoration: 'none' }}>{item.label}</a>)}</nav>
        </div>
    </header>
