...
export default function HeaderV12({ content, tokens }: ThemeComponentProps) {
    const projectName = (content.projectName as string) || (content.businessName as string) || 'Spectrum Prime';
    ...
    <header style={{ background: sectionBg, borderBottom: border, fontFamily: tokens.font }}>
        <div style={{ maxWidth: '980px', margin: '0 auto', padding: '10px 16px', display: 'grid', gap: '8px', justifyItems: 'center' }}>
            <a href="#hero" style={{ color: titleColor, textDecoration: 'none', fontWeight: 800 }}>{projectName}</a>
            <nav style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', justifyContent: 'center' }}>{navItems.map((item) => <a key={item.label + item.href} href={item.href} style={{ color: muted, textDecoration: 'none' }}>{item.label}</a>)}</nav>
            <a href={ctaLink} style={{ textDecoration: 'none', color: '#fff', background: tokens.primary, borderRadius: '8px', padding: '8px 14px' }}>{ctaText}</a>
        </div>
    </header>
