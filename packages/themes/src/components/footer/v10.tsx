...
export default function FooterV10({ content, tokens }: ThemeComponentProps) {
    const businessName = (content.businessName as string) || (content.projectName as string) || 'Aurora Atelier';
    ...
    return (
        <footer id="footer" style={{ background: sectionBg, padding: '58px 16px 24px', borderTop: border, fontFamily: tokens.font }}>
            <div style={{ maxWidth: '1080px', margin: '0 auto', border, background: card, borderRadius: '16px', padding: '12px' }}><h3 style={{ margin: 0, color: titleColor }}>{businessName}</h3><p style={{ margin: '8px 0 0 0', color: muted }}>{text}</p><nav style={{ marginTop: '10px', display: 'flex', gap: '10px', flexWrap: 'wrap' }}>{columns.flatMap((column) => column.links).map((link) => <a key={link.label + link.href} href={link.href} style={{ color: titleColor, textDecoration: 'none', borderBottom: `1px solid ${tokens.primary}` }}>{link.label}</a>)}</nav></div>
