...
export default function FooterV9({ content, tokens }: ThemeComponentProps) {
    const businessName = (content.businessName as string) || (content.projectName as string) || 'Velocity VSL';
    ...
    return (
        <footer id="footer" style={{ background: sectionBg, padding: '58px 16px 24px', borderTop: border, fontFamily: tokens.font }}>
            <div style={{ maxWidth: '1140px', margin: '0 auto', display: 'grid', gap: '10px', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))' }}><article style={{ border, background: card, borderRadius: '12px', padding: '10px' }}><h3 style={{ margin: 0, color: titleColor }}>{businessName}</h3><p style={{ margin: '8px 0 0 0', color: muted }}>{text}</p></article>{columns.slice(0,2).map((column) => <article key={column.title} style={{ border, background: card, borderRadius: '12px', padding: '10px' }}><h4 style={{ margin: 0, color: titleColor }}>{column.title}</h4><ul style={{ margin: '8px 0 0 0', padding: 0, listStyle: 'none', display: 'grid', gap: '6px' }}>{column.links.map((link) => <li key={link.label}><a href={link.href} style={{ color: muted, textDecoration: 'none' }}>{link.label}</a></li>)}</ul></article>)}</div>
