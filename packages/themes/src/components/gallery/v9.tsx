...
export default function GalleryV9({ content, tokens }: ThemeComponentProps) {
    const title = (content.title as string) || 'Gallery Velocity VSL';
    ...
    <div style={{ maxWidth: '1120px', margin: '0 auto', display: 'grid', gap: '10px', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))' }}>
        {images.map((src, index) => <img key={index} src={src} alt={`Gallery ${index + 1}`} style={{ width: '100%', height: '220px', objectFit: 'cover', borderRadius: '14px', border, transform: index % 2 === 0 ? 'translateY(0)' : 'translateY(12px)' }} />)}
    </div>
