...
export default function GalleryV10({ content, tokens }: ThemeComponentProps) {
    const title = (content.title as string) || 'Gallery Aurora Atelier';
    ...
    <div style={{ maxWidth: '1080px', margin: '0 auto' }}>
        ...
        <p style={{ margin: '8px 0 0 0', color: muted }}>{subtitle}</p>
        <div style={{ marginTop: '10px', display: 'grid', gap: '10px' }}>
            {images.map((src, index) => <img key={index} src={src} alt={`Gallery ${index + 1}`} style={{ width: '100%', height: index % 2 === 0 ? '220px' : '180px', objectFit: 'cover', borderRadius: '12px', border }} />)}
        </div>
    </div>
