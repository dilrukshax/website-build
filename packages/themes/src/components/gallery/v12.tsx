...
export default function GalleryV12({ content, tokens }: ThemeComponentProps) {
    const title = (content.title as string) || 'Gallery Spectrum Prime';
    ...
    <div style={{ maxWidth: '980px', margin: '0 auto' }}>
        ...
        <div style={{ marginTop: '10px', display: 'grid', gap: '10px', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))' }}>
            {images.slice(0, 4).map((src, index) => <img key={index} src={src} alt={`Gallery ${index + 1}`} style={{ width: '100%', height: index === 0 ? '280px' : '220px', objectFit: 'cover', borderRadius: '14px', border }} />)}
        </div>
    </div>
