...
export default function ProductV12({ content, styles, tokens, context }: ThemeComponentProps) {
    ...
    return (
        <section id="products" style={{ background: sectionBg, padding: 'clamp(72px, 10vw, 120px) 16px', fontFamily: tokens.font }}>
            <div style={{ maxWidth: '980px', margin: '0 auto', border, background: card, borderRadius: '16px', padding: '12px' }}><h2 style={{ margin: 0, color: titleColor }}>{title}</h2><p style={{ margin: '8px 0 0 0', color: muted }}>{subtitle}</p><div style={{ marginTop: '10px', display: 'grid', gap: '8px' }}>{visible.map((product, i) => <article key={product.id || i} style={{ border, borderRadius: '10px', padding: '10px' }}><h3 style={{ margin: 0, color: titleColor }}>{product.title}</h3><p style={{ margin: '6px 0 0 0', color: muted }}>{product.desc}</p></article>)}</div></div>
        </section>
    );
}
