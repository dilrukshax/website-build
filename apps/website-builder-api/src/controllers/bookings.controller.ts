    ...
    static async create(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            if (Number.isNaN(startTime.getTime()) || Number.isNaN(endTime.getTime())) {
                throw new AppError(ERROR_CODES.VALIDATION_ERROR, 'Invalid booking time range', 400);
            }
            if (endTime.getTime() <= startTime.getTime()) {
                throw new AppError(ERROR_CODES.VALIDATION_ERROR, 'endTime must be after startTime', 400);
            }

            await PlanPolicyService.assertCanCreateBooking(tenantId, instanceId, startTime);
